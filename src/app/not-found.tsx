...
export default function NotFound() {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between font-sans selection:bg-emerald-500 selection:text-white" dir="rtl">
      {/* Top Simple Bar */}
      <header className="bg-[#070B15] text-white border-b border-slate-800 py-4 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <span className="font-extrabold text-xl text-white">نگارش یار</span>
          </Link>
          <Link
            href="/services"
            className="text-xs sm:text-sm text-[#E5C158] hover:underline font-bold"
          >
            مشاهده خدمات
          </Link>
        </div>
      ...
      <footer className="bg-[#070B15] text-slate-400 py-6 text-center text-xs border-t border-slate-800">
        سامانه هوشمند خدمات حقوقی و اداری نگارش یار | پشتیبانی: ۰۹۹۱۵۱۴۷۷۸۹
      </footer>
    </div>
  );
}
